...
$output = '';
if(isset($_POST["query"]) && isset($_POST['mode']) )
{
  $batchmode = mysqli_real_escape_string($connection, $_POST["mode"]);
...
WHERE (course LIKE '%".$search."%' 
  ...
  OR mode LIKE '%".$search."%' 
  OR category LIKE '%" . $search . "%') AND mode = '$batchmode' ORDER BY batch_date DESC
 ";}
else
{
